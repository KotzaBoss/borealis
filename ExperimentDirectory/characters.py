...
from .combat import CombatOutput
from .cursed_functions import cmdline_input, mem_output, err_output
from .damage_types import DMG_TYPES
from .races import RACES
from .utils import init_stat_roll, death_save_roll
...
def set_skills(stats, class_):
    try:
        ...
        mem_output(f"choose {CLASSES[class_]['skill_prof']['num']} from\n"
...
def choose_name():
    name = cmdline_input("What is your characters name: ", prompt=False)
    # some checking
    return name


def choose_race(name):
    race = cmdline_input(f"What is {name}'{'' if name[-1] in ('s', 'S') else 's'} race: ", prompt=False)
    while True:
        if race not in RACES:
            race = cmdline_input(f"Unavailable race try again: ", prompt=False)
        else:
            return race


def choose_class(name):
    class_ = cmdline_input(f"What is {name}'{'' if name[-1] in ('s', 'S') else 's'} class: ", prompt=False)
    while True:
        if class_ not in CLASSES.keys():
            # class_ = cmdline_input(f"Unavailable class try again: ", prompt=False)
            class_ = cmdline_input(f"{CLASSES.keys()} - {class_} ", prompt=False)
        else:
            return class_
